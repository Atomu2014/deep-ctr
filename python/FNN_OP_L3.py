...
class FNN_OP_L3:
    ...
    def forward(self, N, M, v_wts, c_ids, c_wts, sp_inds, act_func='tanh', drop_out=False):
        ...
        tf.matmul(tf.reshape(mbd[0][0, :], [-1, 1]), tf.reshape(mbd[1][0, :], [1, -1]))
        p_mbd = tf.reduce_sum(tf.concat(1, [tf.concat(0, [tf.reshape(tf.matmul(tf.reshape(mbd[_i][_k, :], [-1, 1]),
                                                                               tf.reshape(mbd[_j][_k, :], [1, -1])),
                                                                     [1, 1, -1])
                                                          for _k in range(N)])
                                            for _i in range(len(mbd) - 1) for _j in range(_i + 1, len(mbd))]), 1)
