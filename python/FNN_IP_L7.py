...
class FNN_IP_L7:
    def __init__(self, cat_sizes, offsets, batch_size, _rch_argv, _init_argv, _ptmzr_argv, _reg_argv, mode, eval_size):
        ...
        with self.graph.as_default():
            X_dim, X_feas, rank, h1_dim, h2_dim, h3_dim, h4_dim, h5_dim, h6_dim, h7_dim, act_func = _rch_argv
            mbd_dim = X_feas * (rank + 1) + X_feas * (X_feas - 1) / 2 + 1
            self.log = 'input dim: %d, features: %d, rank: %d, embedding: %d, h1: %d, h2: %d, h3: %d, h4: %d, h5: %d, h6: %d, h7: %d, activate: %s' % \
                       (X_dim, X_feas, rank, mbd_dim, h1_dim, h2_dim, h3_dim, h4_dim, h5_dim, h6_dim, h7_dim, act_func)
            ...
            self.h7_w = tf.Variable(var_map['h7_w'])
            self.h7_b = tf.Variable(var_map['h7_b'])
            self.h8_w = tf.Variable(var_map['h8_w'])
            self.h8_b = tf.Variable(var_map['h8_b'])
    ...
    def forward(self, N, M, v_wts, c_ids, c_wts, sp_inds, act_func='tanh', drop_out=False):
        ...
        if drop_out:
            ...
            l7 = tf.matmul(tf.nn.dropout(activate(act_func, l6), keep_prob=self._keep_prob), self.h6_w) + self.h6_b
            l8 = tf.matmul(tf.nn.dropout(activate(act_func, l7), keep_prob=self._keep_prob), self.h7_w) + self.h7_b
            yhat = tf.matmul(tf.nn.dropout(activate(act_func, l8), keep_prob=self._keep_prob), self.h8_w) + self.h8_b
        else:
