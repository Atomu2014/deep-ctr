...
class FNN:
    def __init__(self, cat_sizes, offsets, batch_size, eval_size, X_dim, X_feas, eval_cols, eval_wts, rank, _min_val,
        ...
        with self.graph.as_default():
            self.fm_w = tf.Variable(tf.random_uniform([X_dim, 1], minval=_min_val, maxval=_max_val, seed=_seeds[0]))
            self.fm_v = tf.Variable(tf.random_uniform([X_dim, rank], minval=_min_val, maxval=_max_val, seed=_seeds[1]))
            ...
            self.loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits, self.lbl_hldr)) + _lambda * (
                ...
                tf.reduce_sum(tf.abs(self.fm_w)) + tf.reduce_sum(tf.abs(self.fm_v)) + tf.abs(self.fm_b) + tf.reduce_sum(
            ...
            self.eval_logits = self.forward(eval_size, X_feas, eval_wts[:, :13], eval_cols[:, 13:] - offsets,
